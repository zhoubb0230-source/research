...
```jsonc
{
  "name":        "scope-definer",          // 不可变，且名字永不复用（含建失败的）
  "description": "🔭 范围界定专家 —— 负责骨架的 ② 枚举",
  "prompt":      <.dsh/PROMPTS/scope-definer.md 正文>,   // ← 人格在这里绑上去
...
low 档是刻意的，但枚举是唯一不可事后修复的动作 —— 首轮跑完必须人工抽查出局池。

## 在 dsh 上落不下去的约束

`../../../expert-team/experts/scope-definer/expert.yaml` 里的 `tools.need` / `tools.deny`
