...
```jsonc
{
  "name":        "arbiter",          // 不可变，且名字永不复用（含建失败的）
  "description": "⚖️ 争议仲裁专家 —— 负责骨架的 ⑧ 仲裁",
  "prompt":      <.dsh/PROMPTS/arbiter.md 正文>,   // ← 人格在这里绑上去
...
按证据等级裁决。禁止投票、禁止取中值、禁止自行补证。

## 在 dsh 上落不下去的约束

`../../../expert-team/experts/arbiter/expert.yaml` 里的 `tools.need` / `tools.deny`
