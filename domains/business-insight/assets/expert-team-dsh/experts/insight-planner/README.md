...
```jsonc
{
  "name":        "insight-planner",          // 不可变，且名字永不复用（含建失败的）
  "description": "🗺️ 洞察规划专家 —— 负责骨架的 ① 界定（含分诊）",
  "prompt":      <.dsh/PROMPTS/insight-planner.md 正文>,   // ← 人格在这里绑上去
...
服务入口。分诊有终止出口：判为检索需求就直接拒绝，不启动专家团。

## 在 dsh 上落不下去的约束

`../../../expert-team/experts/insight-planner/expert.yaml` 里的 `tools.need` / `tools.deny`
